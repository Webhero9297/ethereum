...
contract TokenICO is Owned, Token {
	...
	function TokenICO(address beneficiaryAccount) 
    	Token(tokenName, tokenSymbol, decimalUnits, initialTokens) {   
            ...
            setMinimumInvestment(10); 
            setPrice(2000); 
            setReserveTokens(58000000); 
    ...
    function setBonusPeriod(uint256 newBonusPeriod) onlyOwner returns (uint256) {
        bonusPeriod = newBonusPeriod.mul(1 weeks); 
        return bonusPeriod; 
    }
    ...
    function setReserveTokens(uint256 amount) internal {
        ...
        reservedTokens2 = multiplier.mul(23000000); 
    }

    //Allows owner to start the crowdsale from the time of execution until a specified stopTime
    function startSale(uint256 saleStart, uint256 saleStop, uint256 bonusPeriodLasts, 
                uint256 holdTokens1, uint256 holdTokens2, uint256 saleLimit) onlyOwner returns (bool success) {
        require(saleStop > now);  
        startTime = saleStart; 
        stopTime = saleStop; 
        setBonusPeriod(bonusPeriodLasts);    
        release1Date = holdTokens1; 
        release2Date = holdTokens2; 
